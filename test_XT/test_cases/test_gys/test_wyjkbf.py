...
class test_wyjkbf():
    ...
    @allure.story("选择电子债权凭证我要借款--部分融资")
    def test_wyjkpart(self, get_driver):
        # 用户登录
        ...
        for div in divs:
            ...
            for fdjg in fdjgs:
                # 英大放贷机构下的电子债权凭证
                if fdjg.text == '放贷机构：英大国际信托有限责任公司':
                    ...
                    for tr in table.find_elements_by_tag_name('tr'):
                        ...
                        rzpz = p_element1.text  # 获取第一列的 'p' 标签文本
                        if rzpz == '电子债权凭证融资':
                            last_cell.click()
        ...
        #获取当前最大可借金额
        reqmoney=get_driver.get_element('mytoloan', 'loanmoney')
        maxreqmoney=reqmoney.get_attribute('value')
        newreqmoney=float(maxreqmoney) - 100.0
        reqmoney.clear()
        reqmoney.send_keys(str(newreqmoney))
        # 点击申请借款下一步
